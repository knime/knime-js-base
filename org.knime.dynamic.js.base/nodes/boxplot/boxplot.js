(boxplot_namespace = function() {
    ...
    function createControls(controlsContainer) {
        if (_representation.options.enableViewControls) {
            ...
            if ((_representation.options.conditional || !_representation.options.multi)
                && _representation.options.enableColumnSelection) {
    ...
    function drawChart(resizing) {
        if (_representation.options.conditional) {
            _data = _representation.inObjects[0][_value.options.numCol];
        } else if (_representation.options.multi) {
        ...
        .data(d3.entries(_data), function(d) {
            return (!_representation.options.conditional && !_representation.options.multi) ? "__dummy__" : d.key;
        });
