/* eslint-env jquery */
/* global checkMissingData:false, noUiSlider:false, wNumb:false*/
/*
 * ------------------------------------------------------------------------
 ...
 *
 * History
 *   May 27, 2019 (Daniel Bogenrieder, KNIME.com Gmbh, Konstanz, Germany): created
 */
window.knimeSliderWidget = (function () {
    ...
    function setNumberFormatOptions(settings) {
        var key;
        ...
        if (settings.pips && settings.pips.format) {
            for (key in settings.pips.format) {
    ...
    sliderWidget.init = function (representation) {
        ...
        if (settings.tooltips && settings.tooltips.length > 0) {
            ...
            if (settings.orientation === 'vertical') {
                ...
                sliderContainer.css('padding-left', paddingFactor * maxTipWidth + 'px');
            } else {
                // TODO: calculate based on tooltip height?
                sliderContainer.css('padding-top', '38px');
                padSide = Math.max(parseFloat(getComputedStyle(sliderContainer.get(0)).paddingLeft),
                    maxTipWidth / 2) + 'px';
    ...
    sliderWidget.validate = function () {
        if (!viewValid) {
            return false;
        }
        ...
        if (!$.isNumeric(value)) {
            sliderWidget.setValidationErrorMessage('The set value is not a double');
            return false;
        }
        value = parseFloat(value);
        if (viewRepresentation.usemin && value < min) {
            sliderWidget.setValidationErrorMessage('The set double ' + value +
            ...
            sliderWidget.setValidationErrorMessage(null);
            return true;
        }
    };
    
    sliderWidget.setValidationErrorMessage = function (message) {
        if (!viewValid) {
            return;
        }
        if (message === null) {
