/* eslint-env jquery */
/* global checkMissingData:false, callUpdate:false, radioButtonSingleSelection:false, listSingleSelection:false
 dropdownSingleSelection:false */
/*
 * ------------------------------------------------------------------------
 ...
 */
window.knimeValueSelectionWidget = (function () {
    
    var valueSelection = {
        version: '2.0.0'
    };
    valueSelection.name = 'KNIME Value Selection Widget';
    ...
    valueSelection.init = function (representation) {
        ...
        if (representation.possibleValues === null) {
            ...
            if (columnSelection === '' || !representation.possibleValues.hasOwnProperty(columnSelection)) {
                for (col in representation.possibleValues) {
                    columnSelection = col;
                    break;
                }
            }
            if (!representation.lockColumn) {
                ...
                qfdiv.append($('<br>'));
                for (key in representation.possibleValues) {
    ...
    valueSelection.validate = function () {
        if (!viewValid) {
            return false;
        }
        return true;
    ...
    valueSelection.setValidationErrorMessage = function (message) {
        if (!viewValid) {
            return;
        }
        // TODO: display error
