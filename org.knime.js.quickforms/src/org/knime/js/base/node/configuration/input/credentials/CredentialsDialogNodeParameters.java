...
public class CredentialsDialogNodeParameters extends ConfigurationNodeSettings {
    ...
    static final class CredentialsOverwrittenByValueMessage
        ...
        protected String valueToString(final CredentialsDialogNodeValue value) {
            final var passwordValue =
                value.getPassword() == null || value.getPassword().isEmpty() ? "<not set>" : "<set>";
            return m_enableUsernameFieldSupplier.get() != null && m_enableUsernameFieldSupplier.get()
                ? String.format("user '%s', password %s", value.getUsername(), passwordValue)
    ...
    static final class CredentialsParametersSavedPrior52Persistor
        ...
        public CredentialsParametersSavedPrior52 load(final NodeSettingsRO settings) throws InvalidSettingsException {
            if (settings.containsKey(CredentialsNodeValue.CFG_CREDENTIALS_VALUE_PARENT)) {
                return new CredentialsParametersSavedPrior52(false);
            }

            final var isSavePassword = CredentialsNodeValue.loadIsSavePasswordFromNodeSettingsInDialog(settings);
            final var usernamePassword =
                CredentialsNodeValue.loadUsernameAndPasswordFromNodeSettingsInDialogPrior52(settings, isSavePassword);
            return new CredentialsParametersSavedPrior52(
                new Credentials(usernamePassword.username(), usernamePassword.password()), isSavePassword, true);
        }
        ...
        public void save(final CredentialsParametersSavedPrior52 param, final NodeSettingsWO settings) {
            ...
            final var isSavePassword = param.m_isSavePassword;
            CredentialsNodeValue.saveIsSavePasswordToNodeSettings(settings, isSavePassword);
            CredentialsNodeValue.saveUsernameAndPasswordToNodeSettingsPrior52(settings,
                param.m_credentials.getUsername(), param.m_credentials.getPassword(), isSavePassword);
        }

        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{CredentialsNodeValue.CFG_USERNAME}, {CredentialsNodeValue.CFG_PASSWORD},
    ...
    static final class CredentialsParametersSavedSince52Persistor
        ...
        public CredentialsParametersSavedSince52 load(final NodeSettingsRO settings) throws InvalidSettingsException {
            ...
            final var isSavePassword = CredentialsNodeValue.loadIsSavePasswordFromNodeSettingsInDialog(settings);
            final var usernamePassword =
                CredentialsNodeValue.loadUsernameAndPasswordFromNodeSettingsInDialogSince52(settings, isSavePassword);
            return new CredentialsParametersSavedSince52(
                new Credentials(usernamePassword.username(), usernamePassword.password()), isSavePassword, false);
        }
        ...
        public void save(final CredentialsParametersSavedSince52 param, final NodeSettingsWO settings) {
            ...
            final var isSavePassword = param.m_isSavePassword;
            CredentialsNodeValue.saveIsSavePasswordToNodeSettings(settings, isSavePassword);
            CredentialsNodeValue.saveUsernameAndPasswordToNodeSettingsSince52(settings,
                param.m_credentials.getUsername(), param.m_credentials.getPassword(), isSavePassword);
        }
