...
public final class SingleEntrySelectionRendererUtil {
    ...
    public static LocalizedControlRendererSpec getWebUIDialogControlSpecByType(
        ...
        if (type.equals(SingleSelectionComponentFactory.DROPDOWN)) {
            return new DropdownRenderer(nodeRep, possibleValues);
        }

        if (type.equals(SingleSelectionComponentFactory.LIST)) {
            return new SingleSelectListBoxRenderer(nodeRep, possibleValues, hasSizeLimit, sizeLimit);
        }

        if (type.equals(SingleSelectionComponentFactory.RADIO_BUTTONS_HORIZONTAL)) {
            return new RadioButtonRenderer(nodeRep, possibleValues, "horizontal");
        }

        if (type.equals(SingleSelectionComponentFactory.RADIO_BUTTONS_VERTICAL)) {
            return new RadioButtonRenderer(nodeRep, possibleValues, "vertical");
        }
