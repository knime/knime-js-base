...
import org.knime.js.base.node.parameters.slider.SliderNodeParametersUtil.LabelAndPipsSection;
...
/**
 * Utility class to create the WebUI dialog for slider pips settings.
 *
...
@Layout(LabelAndPipsSection.class)
public final class SliderPipsSettingsNodeParameters implements NodeParameters {
    ...
    @After(LabelAndPipsSection.class)
    @Section(title = "Label Format")
    public interface LabelFormatSection {
    }

    @Widget(title = "Enable labels & pips",
        description = "Check, to enable the display of pips and labels for the slider.")
    @Persist(configKey = CFG_PIPS_DEFINED)
    @ValueReference(EnableLabelsReference.class)
    boolean m_enableLabels;

    @Persistor(PipsSettingsPersistor.class)
    PipsSettings m_pipsSettings = new PipsSettings();

    @Effect(predicate = ShowLabelSettings.class, type = EffectType.SHOW)
    static final class PipsSettings implements NodeParameters {

        PipsSettings() {
        }

        PipsSettings(final PipsMode mode, final boolean stepped) {
            m_mode = mode;
            m_stepped = stepped;
        }

        PipsSettings(final PipsMode mode, final String values, final boolean stepped) {
            this(mode, stepped);
            if (mode == PipsMode.POSITIONS) {
                m_positionValues = values;
            } else if (mode == PipsMode.VALUES) {
                m_values = values;
            }
        }

        PipsSettings(final PipsMode mode, final int count, final boolean stepped) {
            this(mode, stepped);
            m_countValue = count;
        }

        @Widget(title = "Label & pips mode", description = "The mode after which the labels and pips are displayed.")
        @ValueReference(PipsModeReference.class)
        PipsMode m_mode = PipsMode.RANGE;
    ...
    private static final class PipsModeReference implements ParameterReference<PipsMode> {
    }
    ...
    private static final class IsPositionsMode implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getEnum(PipsModeReference.class).isOneOf(PipsMode.POSITIONS);
        }
    }
    ...
    private static final class IsCountMode implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getEnum(PipsModeReference.class).isOneOf(PipsMode.COUNT);
        }
    }
    ...
    private static final class IsValuesMode implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getEnum(PipsModeReference.class).isOneOf(PipsMode.VALUES);
        }
    }
    ...
    private static final class IsRangeOrStepsMode implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getEnum(PipsModeReference.class).isOneOf(PipsMode.RANGE, PipsMode.STEPS);

        }
    }

    /**
     * We cannot use the {@link SliderPipsSettings.PipMode} directly as enum here as the serialization for the selected
    ...
    enum PipsMode {
        ...
        static PipsMode fromPipMode(final PipMode value) {
            return switch (value) {
                case RANGE -> PipsMode.RANGE;
                case STEPS -> PipsMode.STEPS;
                case POSITIONS -> PipsMode.POSITIONS;
                case COUNT -> PipsMode.COUNT;
                case VALUES -> PipsMode.VALUES;
            };
        }

        PipMode toPipMode() {
            return switch (this) {
                case RANGE -> PipMode.RANGE;
                case STEPS -> PipMode.STEPS;
                case POSITIONS -> PipMode.POSITIONS;
                case COUNT -> PipMode.COUNT;
                case VALUES -> PipMode.VALUES;
            };
        }
    }

    private static final class PipsSettingsPersistor implements NodeParametersPersistor<PipsSettings> {

        @Override
        public PipsSettings load(final NodeSettingsRO settings) throws InvalidSettingsException {
            final var pipMode = settings.getString(CFG_MODE);
            final var pipsMode = PipsMode.fromPipMode(PipMode.valueOf(pipMode));
            if (pipsMode == PipsMode.RANGE || pipsMode == PipsMode.STEPS) {
                return new PipsSettings(pipsMode, false);
            }
            final var values = settings.getDoubleArray(CFG_VALUES);
            final var stepped = settings.getBooleanArray(CFG_STEPPED)[0];
            if (pipsMode == PipsMode.COUNT) {
                return new PipsSettings(pipsMode, (int)values[0], stepped);
            }
            return new PipsSettings(pipsMode, Arrays.toString(values), stepped);
        }

        @Override
        public void save(final PipsSettings param, final NodeSettingsWO settings) {
            settings.addString(CFG_MODE, param.m_mode.toPipMode().name());
            double[] values = null;
            boolean[] stepped = new boolean[]{param.m_stepped};
            if (param.m_mode == PipsMode.COUNT) {
                values = new double[]{param.m_countValue};
            } else if (param.m_mode == PipsMode.POSITIONS || param.m_mode == PipsMode.VALUES) {
                final var valuesString = param.m_mode == PipsMode.POSITIONS ? param.m_positionValues : param.m_values;
                /**
