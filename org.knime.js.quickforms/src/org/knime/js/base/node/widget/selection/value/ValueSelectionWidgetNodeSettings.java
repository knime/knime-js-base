...
/**
 * Settings for the Value Selection Widget node.
 *
 * @author AI Migration
 */
@SuppressWarnings("restriction")
public final class ValueSelectionWidgetNodeSettings implements NodeParameters {

    @Section(title = "Value Selection Configuration")
    ...
    // Value selection specific settings
    enum ColumnType {
        @Label("String")
        STRING("String"),
    ...
    @Widget(title = "Column Type", description = "Type of column to select values from")
    @Layout(SelectionSection.class)
    @RadioButtonsWidget
    @Persistor(ColumnTypePersistor.class)
    ColumnType m_columnType = ColumnType.STRING;

    @Widget(title = "Lock Column", description = "If enabled, prevent changing the column selection")
    @Layout(SelectionSection.class)
    @Persist(configKey = "lockColumn")
    boolean m_lockColumn = false;

    @Widget(title = "Default Column", description = "The column to select values from")
    @Layout(SelectionSection.class)
    @ChoicesProvider(AllColumnsProvider.class)
    @Persistor(DefaultColumnPersistor.class)
    String m_defaultColumn = "";

    @Widget(title = "Selection Type", description = "Type of UI component for value selection")
    @Layout(SelectionSection.class)
    @RadioButtonsWidget
    @Persistor(SelectionTypePersistor.class)
    SelectionType m_selectionType = SelectionType.DROPDOWN;

    @Widget(title = "Default Value", description = "Default selected value")
    @Layout(SelectionSection.class)
    @TextInputWidget
    ...
    static class ColumnTypePersistor implements NodeParametersPersistor<ColumnType> {
        @Override
        public ColumnType load(final NodeSettingsRO settings) throws InvalidSettingsException {
            String type = settings.getString("columnType", "String");
            switch (type) {
                case "Integer":
        ...
        @Override
        public void save(final ColumnType value, final NodeSettingsWO settings) {
            settings.addString("columnType", value.getText());
        }

        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{"columnType"}};
        }
    }
    ...
    static class DefaultColumnPersistor implements NodeParametersPersistor<String> {
        ...
        public String load(final NodeSettingsRO settings) throws InvalidSettingsException {
            return settings.getString("column", "");
        }

        @Override
        public void save(final String value, final NodeSettingsWO settings) {
            settings.addString("column", value != null ? value : "");
        }
