...
final class DateWidgetNodeParametersTest extends DefaultNodeSettingsSnapshotTest {
    ...
    protected DateWidgetNodeParametersTest() {
        super(CONFIG);
    }

    private static final SnapshotTestConfiguration CONFIG = SnapshotTestConfiguration.builder() //
        .testJsonFormsForModel(DateWidgetNodeParameters.class) //
        .testJsonFormsWithInstance(SettingsType.MODEL, () -> readSettings()) //
        .testNodeSettingsStructure(() -> readSettings()) //
        .build();

    private static DateWidgetNodeParameters readSettings() {
        try {
            var path = getSnapshotPath(DateWidgetNodeParametersTest.class).getParent().resolve("node_settings")
                .resolve("DateWidgetNodeParameters.xml");
            try (var fis = new FileInputStream(path.toFile())) {
                var nodeSettings = NodeSettings.loadFromXML(fis);
                return NodeParametersUtil.loadSettings(nodeSettings.getNodeSettings(SettingsType.MODEL.getConfigKey()),
                    DateWidgetNodeParameters.class);
            }
