/* global d3:false, jsfc:false, moment:false, twinlistMultipleSelections:false, DateFormat: false*/
window.knimeLinePlot = (function () {
    ...
    view.init = function (representation, value) {
        ...
        try {
            ...
            var columnKey, col, symbolKey, propertyKey, id;
            ...
            if (_value.selection) {
                for (var selection = 0; selection < _value.selection.length; selection++) {
                    for (col = 0; col < _representation.keyedDataset.columnKeys.length; col++) {
                        ...
                        _keyedDataset.select('selection', _value.selection[selection],
                            _representation.keyedDataset.columnKeys[col]);
                    }
                }
            }
    ...
    buildXYDataset = function () {
        ...
        if (_keyedDataset.rowCount() > 0 && _value.yColumns.length > 0) {
            ...
            var customXYDataset = function (source, xcol, ycols) {
                ...
                var xsyms;
                if (xcol === null) {
                    xsyms = this._extractRowSymbols(source);
    ...
    drawChart = function (layoutContainer) {
        ...
        d3.select(svg).attr('id', 'chart_svg').style('width', chartWidth).style('height', chartHeight).style('display',
            isTooManyColumns ? 'none' : 'block');
    ...
    drawControls = function (layoutContainer) {
        ...
        if (_representation.enableXColumnChange || _representation.enableYColumnChange) {
            ...
            if (_representation.enableYColumnChange) {
                ...
                var controlContainer = d3.select('#' + layoutContainer).insert('table', '#' + containerID + ' ~ *')
                    ...
                    /* .style("width", "100%") */
                    .style('padding', '10px').style('margin', '0 auto').style('box-sizing', 'border-box').style(
                        'font-family', defaultFont).style('font-size', defaultFontSize + 'px').style('border-spacing',
         ...
         * Size:").style("margin-right", "5px"); dotSizeContainer.append("td").append("input") .attr("type", "number")
    ...
    getSelection = function () {
        ...
        for (var j = 0; j < selectionsArray.length; j++) {
            selectionIDs.push(_keyedDataset.rowKey(selectionsArray[j].itemKey));
        }
    ...
    drawSeries = function (ctx, dataArea, plot, dataset, seriesIndex) {
        ...
        for (var i = 0; i < itemCount; i++) {
            ...
            if (y === null) {
                ...
                connect = _representation.missingValueMethod === MISSING_VALUE_METHOD_NO_GAP && connect;
                yMissingValuesCount++;
                continue;
            }
        ...
        // TODO check if itemCount makes sense here
        ctx.setLineColor(this.lookupLineColor(dataset, seriesIndex, itemCount));
        ...
        if (_representation.showWarningInView) {
            if (isEmptyPlot) {
                ...
                if (xMissingValuesCount > 0 && _representation.reportOnMissingValues) {
                    knimeService.setWarningMessage(xMissingValuesCount +
                        ' missing value(s) on the X axis are not shown.', MISSING_VALUES_X_AXIS_NOT_SHOWN);
                } else {
                    knimeService.clearWarningMessage(MISSING_VALUES_X_AXIS_NOT_SHOWN);
                }
    ...
    tooManyColumnsError = function (isMany) {
        ...
        if (document.getElementById(errorSvgId) !== null) {
            ...
            var svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
            document.getElementById(containerID).appendChild(svg);
            d3.select(svg).attr('id', errorSvgId).style('display', 'block').attr('width', 600).attr('height', 100)
                .append('rect').attr('x', 0).attr('y', 0).attr('width', 600).attr('height', 100).attr('fill', 'white');
            d3.select(svg).append('text').text(
                'Error: the Line Plot view does not support more than ' + MAX_COLUMNS + ' columns for y-axis.').attr(
                'x', 20).attr('y', 30).attr('fill', 'red');
        }
    };
