...
window.knimeScatterPlotSelectionAppender = (function () {
    ...
    var initialAxisBounds,
         ...
         */
        drawChart, drawControls, buildXYDataset, select, getRowIndex, getJsfcColor, createDateFormatter,
        checkIncludeOrigin, setChartDimensions, unselectHiddenOrFilteredPoints, publishSelection, resize,
        applyFilter, updateChart, updateTitle, updateSubtitle, setTitles, updateXAxisLabel, updateYAxisLabel,
        selectionChanged, filterChanged, unselect, getSelectionItemsById, extractItemKeyFromRefString,
        isRowIncludedInFilter, setAxisBoundsToValue, legendInfo, createLegend;
    ...
    view.init = function (representation, value) {
        ...
        if (representation.keyedDataset.columnKeys.indexOf(value.xColumn) === -1) {
            d3.select('body').append('p').text(
                'Error: Selected column for x-axis: "' + value.xColumn + '" not available.');
            return;
        }
        ...
        try {
            ...
            var col, columnKey, propertyKey, symbolKey, id;
            ...
            for (var rowIndex = 0; rowIndex < _representation.keyedDataset.rows.length; rowIndex++) {
                ...
                for (col = 0; col < _representation.keyedDataset.columnKeys.length; col++) {
                    columnKey = _representation.keyedDataset.columnKeys[col];
            ...
            d3.select('body').attr('id', 'body').append('div').attr('id', layoutContainer).style('width', '100%')
                .style('height', '100%').style('min-width', minWidth + 'px').style('min-height', minHeight + 'px');
    ...
    buildXYDataset = function () {
        ...
        if (_representation.showWarningInView) {
            if (isEmptyPlot) {
                ...
                knimeService
                    .setWarningMessage(
                        'No chart was generated since the selected pair of data columns has only missing values.\nChoose another data columns or re-run the workflow with different data.',
                        NO_DATA_AVAILABLE);
            } else {
                knimeService.clearWarningMessage(NO_DATA_AVAILABLE);
                if (missingValuesCount > 0 && _representation.reportOnMissingValues) {
                    knimeService.setWarningMessage(missingValuesCount +
                        ' missing or unsupported value(s) are not shown.', MISSING_VALUES_NOT_SHOWN_WARNING_ID);
                }
            }
        }
    ...
    drawChart = function (layoutContainer) {
        ...
        // Assign selection in _keyedDataset is lost when building XYDataset, therefore we need to assing selection
        // directly to XYDataset.
        ...
        d3.select('#' + layoutContainer).append('div').attr('id', containerID).style('width', chartWidth).style(
            'height', chartHeight).style('min-width', minWidth + 'px').style('min-height', minHeight + 'px').style(
            'box-sizing', 'border-box').style('overflow', 'hidden').style('margin', '0');
        ...
        var dateProp;
        if (_value.xColumn) {
            dateProp = _keyedDataset.getColumnProperty(_value.xColumn, 'date');
            if (dateProp) {
        ...
        if (_value.yColumn) {
            dateProp = _keyedDataset.getColumnProperty(_value.yColumn, 'date');
            if (dateProp) {
        ...
        /*
         * if (panEnabled) { var panModifier = new jsfc.Modifier.createModifier(false, false, false, false); if
         * (dragZoomEnabled) { panModifier = new jsfc.Modifier.createModifier(false, true, false, false); } var
         * panHandler = new jsfc.PanHandler(chartManager, panModifier); chartManager.addLiveHandler(panHandler); }
         *
         * var selectionEnabled = _representation.enableSelection; var recSelEnabled =
         * _representation.enableRectangleSelection; var lasSelEnabled = _representation.enableLassoSelection;
         *
         * if (selectionEnabled) { var selectionModifier = new jsfc.Modifier.createModifierWML(true, false, false,
         * true); var clickSelectionHandler = new jsfc.ClickSelectionHandler(chartManager, selectionModifier); if
         * (recSelEnabled) { var rectangleSelectionHandler = new jsfc.RectangleSelectionHandler(chartManager,
         * selectionModifier); var selectionHandler = new jsfc.DualHandler(chartManager, selectionModifier,
         * clickSelectionHandler, rectangleSelectionHandler); chartManager.addLiveHandler(selectionHandler); } else {
         * chartManager.addLiveHandler(clickSelectionHandler); } if (lasSelEnabled) { var polygonSelectionModifier = new
         * jsfc.Modifier.createModifierWML(true, true, false, true); var polygonSelectionHandler = new
         * jsfc.PolygonSelectionHandler(chartManager, polygonSelectionModifier);
    ...
    getJsfcColor = function (colorString) {
        ...
        var color = new jsfc.Color(parseInt(colC[0], 10),
                                    parseInt(colC[1], 10),
                                    parseInt(colC[2], 10),
                                    parseInt(colC[3], 10) * 255);
        return color;
    };

    checkIncludeOrigin = function (axis, margin) {
        var min = axis.getLowerBound();
        var max = axis.getUpperBound();
        ...
        axis.setBounds(min, max, false, true);
        if (min < 0) {
            axis.setBoundsByPercent(-margin, 1.01, false, true);
        } else {
            axis.setBoundsByPercent(0, 1 + margin, false, true);
        }
    };
    ...
    updateChart = function () {
        ...
        if (_representation.enforceOrigin) {
            checkIncludeOrigin(plot.getYAxis(), 0.05);
            checkIncludeOrigin(plot.getXAxis(), 0.0);
            chartManager.refreshDisplay();
        }
    };
    ...
    drawControls = function (layoutContainer) {
        ...
        if (dragZoomEnabled) {
            var zoomButtonClicked = function () {
                d3.selectAll('#knime-service-header .service-button').classed('active', function () {
                    return this.getAttribute('id') === 'scatter-mouse-mode-zoom';
                });
        ...
        if (selectionEnabled) {
            var selectionButtonClicked = function () {
                d3.selectAll('#knime-service-header .service-button').classed('active', function () {
                    return this.getAttribute('id') === 'scatter-mouse-mode-select';
                });
                chartManager._availableLiveMouseHandlers = [];
                ...
                if (recSelEnabled) {
                    ...
                    var selectionHandler = new jsfc.DualHandler(chartManager, selectionModifier, clickSelectionHandler,
                        rectangleSelectionHandler);
                ...
                if (lasSelEnabled) {
                    ...
                    var polygonSelectionHandler = new jsfc.PolygonSelectionHandler(chartManager,
                        polygonSelectionModifier);
                    chartManager.addLiveHandler(polygonSelectionHandler);
                }
            };
            knimeService.addButton('scatter-mouse-mode-select', 'check-square-o', 'Mouse Mode "Select"',
        ...
        if (panEnabled) {
            var panButtonClicked = function () {
                d3.selectAll('#knime-service-header .service-button').classed('active', function () {
                    return this.getAttribute('id') === 'scatter-mouse-mode-pan';
                });
                chartManager._availableLiveMouseHandlers = [];
        ...
        if (_representation.enableTitleChange || _representation.enableSubtitleChange) {
            ...
            if (_representation.enableTitleChange) {
                var chartTitleText = knimeService.createMenuTextField('chartTitleText', _value.chartTitle, updateTitle,
                    false);
        ...
        if (_representation.enableXColumnChange || _representation.enableYColumnChange) {
            ...
            if (_representation.enableXColumnChange) {
                var xSelect = knimeService.createMenuSelect('xColumnSelect', _value.xColumn,
                    _keyedDataset.columnKeys(), function () {
            ...
            if (_representation.enableYColumnChange) {
                var ySelect = knimeService.createMenuSelect('yColumnSelect', _value.yColumn,
                    _keyedDataset.columnKeys(), function () {
        ...
        if (_representation.enableXAxisLabelEdit || _representation.enableYAxisLabelEdit) {
            ...
            if (_representation.enableXAxisLabelEdit) {
                var xAxisText = knimeService.createMenuTextField('xAxisText', _value.xAxisLabel, updateXAxisLabel,
                    false);
        ...
        if (_representation.enableDotSizeChange) {
            ...
            /*
             * var dotSizeContainer = controlContainer.append("tr").style("margin", "5px auto").style("display",
             * "table"); dotSizeContainer.append("td").append("label").attr("for", "dotSizeInput").text("Dot
             * Size:").style("margin-right", "5px"); dotSizeContainer.append("td").append("input") .attr("type",
             * "number") .attr("id", "dotSizeInput") .attr("name", "dotSizeInput") .attr("value", _value.dotSize)
             * .style("font-family", defaultFont) .style("font-size", defaultFontSize+"px");
             */
        }
        if (_representation.enableSwitchLegend && _legendBuilder) {
            // Initialization of _legendBuilder means there's a color model. We need to check it because of backward
            // compatibility:
            // the old executed plots do not have color models, but default value for enableSwitchLegend is true.
            if (pre) {
                knimeService.addMenuDivider();
            }
            var showLegendCheckbox = knimeService.createMenuCheckbox('showLegendCheckbox', _value.showLegend,
                function () {
        ...
        if (_representation.enableSelection) {
            ...
            if (_representation.enableShowSelectedOnly) {
                var showSelectedOnlyCheckbox = knimeService.createMenuCheckbox('showSelectedOnlyCheckbox',
                    _value.showSelectedOnly, function () {
        ...
        if (knimeService.isInteractivityAvailable()) {
            ...
            if (_representation.enableSelection) {
                ...
                var pubSelCheckbox = knimeService.createMenuCheckbox('publishSelectionCheckbox',
                    _value.publishSelection, function () {
                        if (this.checked) {
                ...
                var subSelCheckbox = knimeService.createMenuCheckbox('subscribeSelectionCheckbox',
                    _value.subscribeSelection, function () {
                        if (this.checked) {
                            knimeService.subscribeToSelection(_representation.keyedDataset.id, selectionChanged);
                        } else {
            ...
            if (_representation.subscriptionFilterIds && _representation.subscriptionFilterIds.length > 0) {
                ...
                var subFilIcon = knimeService.createStackedIcon('filter', 'angle-double-right', 'faded right sm',
                    'left bold');
                var subFilCheckbox = knimeService.createMenuCheckbox('subscribeFilterCheckbox', _value.subscribeFilter,
                    function () {
                        if (this.checked) {
                            knimeService.subscribeToFilter(_representation.keyedDataset.id, filterChanged,
                                _representation.subscriptionFilterIds);
                        } else {
    ...
    selectionChanged = function (data) {
        ...
        if (data.changeSet) {
            ...
            if (data.changeSet.added) {
                for (var j = 0; j < data.changeSet.added.length; j++) {
                    var addedId = getRowIndex(data.changeSet.added[j]);
            ...
            if (oldSelectionItems) {
                ...
                for (var k = 0; k < oldSelectionItems.length; k++) {
                    oldSelection.push(oldSelectionItems[k].itemKey);
                    // at the same time, if an item from the oldSelection is not presented in the newSelection, then it
                    // was unselected => removed
                    if (newSelection.indexOf(oldSelectionItems[k].itemKey) === -1) {
                        removedIds.push(oldSelectionItems[k].itemKey);
                    }
                }
            }
            ...
            for (var n = 0; n < newSelection.length; n++) {
                select(dataset, newSelection[n]);
                // at the same time, if an item from the newSelection is not presented in the oldSelection, then it was
                // selected => added
                if (oldSelection.indexOf(newSelection[n]) === -1) {
                    addedIds.push(newSelection[n]);
                }
            }
        }
        ...
        d3.selectAll('circle').each(function () {
            ...
            for (var m = 0; m < removedIds.length; m++) {
                if (itemKey === removedIds[m]) {
    ...
    getSelection = function () {
        ...
        for (var p = 0; p < selections.length; p++) {
            if (selections[p].id === SELECTION_ID) {
                selectionsArray = selections[p].items;
                break;
            }
        }
        var selectionIDs = [];
    ...
    /**
     * Select the point by its row index in the given dataset without notifying the listeners, which means the point is
     * only added to the selection object, but not visually redrawn
     *
    ...
    /**
     * Unselect the point by its row index in the given dataset without notifying the listeners, which means the point
     * is only removed from the selection object, but not visually redrawn
     *
    ...
    /**
     * Getting the row index in dataset based on the provided row key. For some internal reason, the row index must be
    ...
    getRowIndex = function (rowKey) {
        ...
        return String(indexRowkeyMap.indexOf(rowKey));
    };
    ...
    setAxisBoundsToValue = function () {
        ...
        if (xMin !== initialAxisBounds.xMin || xMax !== initialAxisBounds.xMax || yMin !== initialAxisBounds.yMin ||
            yMax !== initialAxisBounds.yMax) {
    ...
    createLegend = function (plot, anchor, orientation, style) {
        ...
        /*
         * var title = new jsfc.TextElement().setFont(me._font); var item = new jsfc.GridElement(); item.add(title,
         * "R1", "C1"); result.add(item);
         */
    ...
    extractXYDatasetFromColumns2D = function (dataset, xcol, ycol, seriesKey) {
