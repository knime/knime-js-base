 ...
 * @since 2.6
 */
final class ColumnSelectionInputQuickFormPanel
    extends QuickFormConfigurationPanel
        <ColumnSelectionInputQuickFormValueInConfiguration> {
     ...
     */
    ColumnSelectionInputQuickFormPanel(
            final ColumnSelectionInputQuickFormInConfiguration cfg) {
        super(new BorderLayout());
        final JLabel label = new JLabel(cfg.getDescription());
        add(label, BorderLayout.NORTH);
        m_valueField = new DialogComponentStringSelection(
                new SettingsModelString("column-selection", null),
                cfg.getLabel(), cfg.getChoices());
        add(m_valueField.getComponentPanel(), BorderLayout.CENTER);
    ...
    public void saveSettings(
            final ColumnSelectionInputQuickFormValueInConfiguration config)
    ...
    public void updateQuickFormInElement(final AbstractQuickFormInElement e) throws InvalidSettingsException {
        StringOptionInputQuickFormInElement cast = AbstractQuickFormInElement.cast(
                    StringOptionInputQuickFormInElement.class, e);
        cast.setValue(getValue());
    }
    
    private String getValue() {
        return ((SettingsModelString) m_valueField.getModel()).getStringValue();
    }
}
