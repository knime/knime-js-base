// JavaScript Document
kc.lineChart = function() {
	
	var lineChart = {};
	lineChart.name = "kc_lineChart";
	...
	lineChart.init = function(p_container) {
		container = "#" + p_container;
		hiliteHandler = kc.getExtension("hilite");
		...
		valueSet = color.domain().map(function(name, i) {
			return {
				name: name,
				values: kc.getColumn(numberColsIDs[i])
			};
		});
		x = d3.scale.linear()
			.domain([0, kc.getNumRows()])
			.nice()
			.range([0, width]);
